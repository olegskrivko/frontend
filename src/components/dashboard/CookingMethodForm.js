...
const CookingMethodForm = () => {
  ...
  const handleFormSubmit = async (event) => {
    ...
    try {
      ...
      const response = await axios.post(`${BASE_URL}/cooking-methods`, {
        name,
        description,
      });

      console.log("Cooking method created:", response.data);

      setSuccessMessage("Cooking method created successfully!");
      setName("");
      setDescription("");
      setErrors({});
    } catch (error) {
      if (error.response && error.response.data && error.response.data.errors) {
        ...
        console.error("Error creating cooking method:", error.message);
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <>
      <form onSubmit={handleFormSubmit}>
        <TextField label="Cooking Method Name" variant="outlined" fullWidth value={name} onChange={(e) => handleFieldChange("name", e.target.value)} margin="normal" error={!!errors.name} helperText={errors.name} />
        ...
        <Button type="submit" variant="contained" color="primary" disabled={loading}>
          {loading ? <CircularProgress size={24} /> : "Create Cooking Method"}
        </Button>
      </form>

...
const IngredientCategoryList = () => {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const response = await axios.get(`${BASE_URL}/ingredient-categories`);
        setCategories(response.data);
        // const parentCategories = response.data.filter((category) => category.parentCategory === null);
  ...
  const handleEditSave = async () => {
    try {
      const response = await axios.put(`${BASE_URL}/ingredient-categories/${selectedCategory._id}`, {
        name: editName,
        description: editDescription,
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`${BASE_URL}/ingredient-categories/${id}`);
      setCategories((prevCategories) => prevCategories.filter((category) => category._id !== id));
    } catch (error) {
      console.error("Error deleting category:", error.message);
      setError("Error deleting category");

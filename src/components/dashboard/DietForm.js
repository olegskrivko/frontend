...
const DietForm = () => {
  ...
  const handleFormSubmit = async (event) => {
    ...
    try {
      ...
      const response = await axios.post(`${BASE_URL}/diets`, {
        name,
        description,
      });
      ...
      if (error.response && error.response.data && error.response.data.errors) {
        ...
        console.error("Error creating diet:", error.message);
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <>
      <form onSubmit={handleFormSubmit}>
        <TextField label="Diet Name" variant="outlined" fullWidth value={name} onChange={(e) => handleFieldChange("name", e.target.value)} margin="normal" error={!!errors.name} helperText={errors.name} />
        ...
        <Button type="submit" variant="contained" color="primary" disabled={loading}>
          {loading ? <CircularProgress size={24} /> : "Create Diet"}
        </Button>
      </form>

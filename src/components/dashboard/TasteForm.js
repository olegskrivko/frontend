...
const TasteForm = () => {
  ...
  const handleFormSubmit = async (event) => {
    ...
    try {
      ...
      const response = await axios.post(`${BASE_URL}/tastes`, {
        name,
        description,
      });

      console.log("Taste created:", response.data);

      setSuccessMessage("Taste created successfully!");
      setName("");
      setDescription("");
      setErrors({});
    } catch (error) {
      if (error.response && error.response.data && error.response.data.errors) {
        ...
        console.error("Error creating taste:", error.message);
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <>
      <form onSubmit={handleFormSubmit}>
        <TextField label="Taste Name" variant="outlined" fullWidth value={name} onChange={(e) => handleFieldChange("name", e.target.value)} margin="normal" error={!!errors.name} helperText={errors.name} />
        ...
        <Button type="submit" variant="contained" color="primary" disabled={loading}>
          {loading ? <CircularProgress size={24} /> : "Create Taste"}
        </Button>
      </form>

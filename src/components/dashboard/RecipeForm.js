...
const RecipeForm = () => {
  ...
  useEffect(() => {
    const fetchOccasions = async () => {
      try {
        const response = await axios.get(`${BASE_URL}/occasions`);
  ...
  useEffect(() => {
    const fetchCookingMethods = async () => {
      try {
        const response = await axios.get(`${BASE_URL}/cooking-methods`);
  ...
  const handleTipChange = (index, field, value) => {
    ...
    updatedTips[index][field] = value;
    setTips(updatedTips);
  };

  const handleAddTip = () => {
    setTips([...tips, { title: "", description: "" }]);
  };
  ...
  const handleCheckboxChange = (dietId) => {
    // Toggle the selection of the diet
      ...
      <form onSubmit={handleFormSubmit}>
        ...
        {tips.map((tip, index) => (
          <Box key={index} display="flex" alignItems="center">
            <TextField label={`Tip ${index + 1} Title`} variant="outlined" fullWidth value={tip.title} onChange={(event) => handleTipChange(index, "title", event.target.value)} margin="normal" />
            <TextField label={`Tip ${index + 1} Description`} variant="outlined" fullWidth value={tip.description} onChange={(event) => handleTipChange(index, "description", event.target.value)} margin="normal" />
            <IconButton onClick={() => handleRemoveTip(index)}>
              <CloseIcon />
            </IconButton>
          </Box>
        ))}

        <Button variant="contained" color="primary" onClick={handleAddTip} disabled={loading}>
          Add Tip
        </Button>
        {/* Tags Input */}
        <TextField label="Tags (comma-separated)" variant="outlined" fullWidth value={newTag} onChange={handleTagChange} margin="normal" />
        <Button variant="contained" color="primary" onClick={handleAddTag} disabled={loading}>
          Add Tag
        </Button>
        <div style={{ marginTop: 10 }}>
          {tags.map((tag, index) => (
            <Chip key={index} label={tag} onDelete={() => handleRemoveTag(tag)} color="primary" style={{ marginRight: 5, marginBottom: 5 }} />
          ))}
        </div>
        {/* Tags Input */}
        ...
        {allDiets.map((diet) => (
          <div key={diet._id}>
            <input type="checkbox" id={diet._id} checked={selectedDiets.includes(diet._id)} onChange={() => handleCheckboxChange(diet._id)} />
            <label htmlFor={diet._id}>{diet.name}</label>
          </div>
        ))}
        <br /> */}
        ...
        <Grid item xs={12}>
          <FormControl component="fieldset">
            <FormGroup>
              {allDiets.map((diet) => (
                <FormControlLabel key={diet._id} control={<Checkbox checked={selectedDiets.includes(diet._id)} onChange={() => handleCheckboxChange(diet._id)} name={diet.name} />} label={diet.name} />
              ))}
            </FormGroup>
          </FormControl>
        </Grid>

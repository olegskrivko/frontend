...
function EquipmentPanel({ analyzedInstructions }) {
  ...
  const allEquipment = analyzedInstructions.flatMap((instruction) => instruction.steps.flatMap((step) => step.equipment));
      ...
      {nutritionData && (
        <ul>
          {nutritionData.map((item) => (
            <li key={item.id}>
              {item.name} {item.amount} Percent Of Daily Needs: {item.percentOfDailyNeeds} Unit: {item.unit}
            </li>
          ))}
        </ul>
      )}
  ...
  return (
    <Box sx={{ width: "100%" }}>
      ...
      <CustomTabPanel value={value} index={1}>
        <Typography variant="body1" style={{ fontWeight: "bold", marginBottom: "0.6rem" }}>
          Equipment
        </Typography>

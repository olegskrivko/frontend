...
function IconLabelTabs({ recipe, servings, setServings }) {
  console.log("servings, setServings", servings + setServings);
  const [value, setValue] = useState(0);
  //const [servings, setServings] = useState(recipe.servings || 1); // Initial servings
  const handleServingsChange = (event) => {
    const newServings = event.target.value;
    if (!isNaN(newServings)) {
      setServings(parseInt(newServings));
    }
  };

  // Function to calculate updated ingredient quantities based on servings
  const calculateUpdatedIngredients = (ingredients, servings, setServings) => {
    console.log("servingsservingsservings", servings);
    console.log("recipe.servingsrecipe.servings", recipe.servings);
    return ingredients.map((ingredient) => ({
      ...ingredient,
      quantity: (ingredient.quantity * servings) / recipe.servings, // Adjust quantity based on servings
    }));
  };

  // Calculate updated ingredients
  const updatedIngredients = calculateUpdatedIngredients(recipe.ingredients, servings, setServings);
  console.log("updatedIngredients", updatedIngredients);
  const handleChange = (event, newValue) => {
    setValue(newValue);
  };

  const Section = ({ ingredients, servings, setServings }) => (
    <div>
        ...
        <div key={ingredientGroup.name}>
          ...
          {ingredientGroup.items.map((ingredient) => (
            <div key={ingredient._id}>
              ...
              {/* {servings}
              {setServings} */}
              {/* <p>{(ingredient.quantity * servings) / recipe.servings}</p> */}
              {/* {console.log(ingredient)}  */}
              <FormControlLabel control={<Checkbox checked color="default" icon={<RadioButtonUncheckedIcon />} checkedIcon={<CheckCircleIcon />} />} label={`${ingredient.ingredient.name} (${((ingredient.quantity / servings) * setServings).toFixed(1)} ${ingredient.unit})`} />
            </div>
          ))}
        </div>
      ))}
  ...
  return (
    <div style={{ margin: "2rem 0" }}>
      ...
      <TabPanel value={value} index={0}>
        {recipe.ingredients && <Section ingredients={updatedIngredients} servings={recipe.servings} setServings={servings} />}
      </TabPanel>
      {/* Tools Tab */}
        ...
        <Grid container spacing={3}>
          ...
          >
            <div style={{ display: "flex", justifyContent: "center", alignItems: "center" }}>
              <TotalNutritionDonutChart ingredients={recipe.ingredients} servings={servings} />
            </div>
          </Grid>
        </Grid>
      </TabPanel>

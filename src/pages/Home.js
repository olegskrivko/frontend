...
import { Box, Typography, Divider, List, ListItemButton, ListItemText, Collapse, ListItem, Card, CardMedia, CardContent } from "@mui/material";
        ...
        <Box>
          {meals.map((meal, index) => (
            <Box key={index} mb={3}>
              ...
              {/* Fetch and map through recipes for the current meal type */}
              {/* <Grid container spacing={2}>
              
                {meal.recipes.slice(0, 4).map((recipe, recipeIndex) => (
                  <Grid item key={recipeIndex} xs={6} sm={6} md={4} lg={3}>
                    <Link to={`/recipes/${recipe._id}`} style={{ textDecoration: "none" }}>
                      <Card>
                        <CardMedia component="img" height="140" image={recipe.coverImage} alt={recipe.title} />
                        <CardContent>
                          <Typography variant="subtitle1" textAlign="center">
                            {recipe.title}
                          </Typography>
